...
Preferences DeviceInfo;
Preferences EnergyInfo;
Flash::Flash() {}
...
void Flash::Begin()
{
    ...
    DeviceInfo.begin("firmware", false); // Use "storage" namespace
    EnergyInfo.begin("energy", false); // Use "energy" namespace
    // Serial.println("NVS Initialized");
    Serial.println("NVS Initialized");
    // if (NVS_ReadString("device_id") == "")
    // {
    //     NVS_WriteString("device_id", "5500760000001");
    // }
...
void Flash::DefaultData()
{
    ...
    delay(1);
    deviceId = NVS_ReadString(&DeviceInfo, "device_id");
    Serial.println("Newdevice ID: " + String(deviceId) + "  real value: " + NVS_ReadString(&DeviceInfo, "device_id"));
    delay(1);
    ...
    delay(1);
    initialUsage = NVS_ReadString(&EnergyInfo, "usage").toFloat();
}
...
// ==== NVS Functions ====
void Flash::NVS_WriteString(Preferences *preferences, const char *key, const String &value)
{
    preferences->putString(key, value);
    Serial.println("Data Written to NVS: " + String(key) + " = " + value);
}

String Flash::NVS_ReadString(Preferences *preferences, const char *key, const String &defaultValue)
{
...
void Flash::NVS_RemoveKey(Preferences *preferences, const char *key)
{
    preferences->remove(key);
    Serial.println("Key Removed from NVS: " + String(key));
}

void Flash::NVS_Clear(Preferences *preferences)
{
    preferences->clear();
    Serial.println("NVS Cleared");
}

void Flash::NVS_Close(Preferences *preferences)
{
    preferences->end();
